"""
Kimi-K2.5 (Moonshot AI) LLM provider implementation.

Uses the OpenAI-compatible API endpoint provided by Moonshot AI.
"""
...
class KimiProvider:
    """Kimi-K2.5 (Moonshot AI) LLM provider.

    Moonshot AI exposes an OpenAI-compatible chat completions API,
    so we use the openai SDK pointed at their base URL.

    Supports:
    - kimi-k2-0711: Latest flagship model
    ...
    async def complete(
        ...
        """Generate a completion using Kimi models.

        Args:
            messages: Conversation history
            model: Model identifier (uses kimi-k2-0711 if None)
        ...
        try:
            ...
            logger.debug(f"Kimi request: model={model}, messages={len(messages)}")
            ...
            logger.debug(
                f"Kimi response: tokens={input_tokens + output_tokens}, "
            ...
            logger.warning(f"Kimi rate limit error: {e}")
            ...
            logger.error(f"Unexpected error in Kimi provider: {e}")
            raise ProviderError(
    ...
    async def complete_stream(
        ...
        """Stream completion tokens from Kimi.
        ...
        try:
            ...
            logger.warning(f"Kimi rate limit during streaming: {e}")
            raise RateLimitError(provider=self.provider_name) from e

        except OpenAIError as e:
            logger.error(f"Kimi API error during streaming: {e}")
            raise ProviderError(provider=self.provider_name, detail=str(e)) from e
